...
###################################
  ...
  sampleId=$( cat $4 | dos2unix | sed $'s/[^[:print:]\t]//g' | tr -d '^M' | sed "${SLURM_ARRAY_TASK_ID}q;d" | cut -f3 -d',' )
  srr=$( cat $4 | dos2unix | sed $'s/[^[:print:]\t]//g' | tr -d '^M' | sed "${SLURM_ARRAY_TASK_ID}q;d" | cut -f1 -d',' )
  numFlies=$( cat $4 | dos2unix | sed $'s/[^[:print:]\t]//g' | tr -d '^M' | sed "${SLURM_ARRAY_TASK_ID}q;d" | cut -f2 -d',' )
